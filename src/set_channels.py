...
async def set_blogchannel(interaction: discord.Interaction, channel: discord.TextChannel):
    ...
    # 🔞 Verificar que el canal destino sea NSFW
    if not channel.nsfw:
        await interaction.followup.send(
            f"{EMOJI_NO} El canal {channel.mention} no es **+18 (NSFW)**.\n"
            "Marca el canal como NSFW en su configuración antes de asignarlo.",
            ephemeral=True
        )
        return
    ...
    try:
        ...
        await interaction.followup.send("❌ Error guardando el canal.")
